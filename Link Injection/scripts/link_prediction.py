...
def balanced_samples(edge_index, num_nodes):
    num_sample = edge_index.shape[1]
    neg_index = negative_sampling(edge_index, num_nodes)
    edge_balanced = torch.cat((neg_index, edge_index), dim=1)
    y_balanced = torch.cat((
        torch.zeros(num_sample, 1).long(), 
        torch.ones(num_sample, 1).long()
    ))
    return edge_balanced, y_balanced.to(edge_index.device)
